...
/* ---------- SAVE SYSTEM ----------
   window.storage は Claude のアーティファクト内でしか使えない特殊APIなので、
   CodePen/GitHub Pages/Electron でも動く標準の localStorage を使う。 */
...
async function loadSave(){
  try{
    ...
    if(raw){ saveData = Object.assign({ unlocked:1, stars:{}, hiddenFound:{}, totalClears:0, adIndex:0 }, JSON.parse(raw)); }
...
async function writeSave(){
  try{ localStorage.setItem(SAVE_KEY, JSON.stringify(saveData)); }catch(e){ /* best effort */ }
}
