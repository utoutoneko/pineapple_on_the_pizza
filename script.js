...
const SECRET_HINT = "ヒント: ふつうにプレイしているだけでは気づけない何かが、ステージの中に隠れている…";
...
STAGE_LOGIC.shift = {
  par: 8,
  init(){
    return { safe:1, t:0, cycle:1.3, round:0, need:5, warnAt: s=>{} };
  },
  update(s, dt, inp){
    checkSecretDoubleTap(s, dt, inp);
    s.t += dt;
  ...
  render(ctx,s){
    drawGround(ctx);
    for(let i=0;i<3;i++){
      const warn = s.secretMode && s.t > s.cycle-1.0;
      ctx.fillStyle = i===s.safe && warn ? 'rgba(255,201,60,.5)' : '#f2e3c6';
      ctx.fillRect(i*(W/3)+6, H*0.4, W/3-12, H*0.3);
    }
    if(typeof s.pos==='number'){ drawPineapple(ctx, s.pos*(W/3)+W/6, H*0.55); }
    ctx.fillStyle='#7A4A26'; ctx.font='bold 12px Nunito'; ctx.fillText('安全地帯へ移動 ROUND '+s.round+'/'+s.need, 12, 22);
  }
};
...
STAGE_LOGIC.cancel = {
  par: 4,
  init(){
    return { gauge:0, done:false };
  },
  update(s, dt, inp){
    checkSecretDoubleTap(s, dt, inp);
    s.gauge += 55*dt;
    if(inp.justDown){
      const lo = s.secretMode ? 55 : 78, hi = s.secretMode ? 99 : 92;
  ...
  render(ctx,s){
    drawGround(ctx);
    ctx.fillStyle='#e7dcc0'; ctx.fillRect(40,H/2-16,W-80,32);
    ctx.fillStyle = s.gauge>78 ? '#FF3B3B':'#FFC93C';
    ctx.fillRect(40,H/2-16,(W-80)*clamp(s.gauge/100,0,1),32);
    ctx.strokeStyle='#7A4A26'; ctx.lineWidth=3; ctx.strokeRect(40,H/2-16,W-80,32);
    ctx.fillStyle='#7A4A26'; ctx.font='bold 13px Nunito'; ctx.fillText('暴走寸前でタップしてキャンセル!', 12, 22);
  }
};
